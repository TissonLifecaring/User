...
- (void) layoutSubviews
{
	...
	if(self.badgeString && ![self.badgeString isEqualToString:@"0"])
	{
        ...
        NSNumber *badgeWidth = [NSNumber numberWithFloat:badgeSize.width + self.badgeHorizPadding];
        NSNumber *badgeHeight = [NSNumber numberWithFloat:badgeSize.height + (self.badgeVertPadding/badgeSize.height)];
        
        [self.badge mas_makeConstraints:^(MASConstraintMaker *make) {
            
            if (self.badgeRelativeView) {
                make.left.equalTo(self.badgeRelativeView.mas_right).with.offset(20);
