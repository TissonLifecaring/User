...
typedef NS_ENUM(NSInteger, GraphSearchMode){
    ...
    GraphSearchModeByThreeDay,
    GraphSearchModeByWeek,
    GraphSearchModeByTwoWeek,
    GraphSearchModeByMonth,
    GraphSearchModeByTwoMonth,
    GraphSearchModeByThreeMonth
};

typedef NS_ENUM(NSInteger, GraphDotType){
    GraphDotTypeDetect = 0,
    GraphDotTypeDrug,
    GraphDotTypeDiet,
};
...
@interface BEMSimpleLineGraphView : UIView <UIScrollViewDelegate,UIGestureRecognizerDelegate>
...
/// Color for the upper limit value
@property (strong, nonatomic) UIColor *hyperColor;

/// Color for the lower limit value
@property (strong, nonatomic) UIColor *hypoColor;
...
/** Sent to the delegate when the user starts Tapping the key point on the graph. The property 'enableTouchReport' must be set to YES
 @param graph The graph object which was touched by the user.
 @param index The closest point from the location the user is currently tapping. return -1 if cannot find the point*/
